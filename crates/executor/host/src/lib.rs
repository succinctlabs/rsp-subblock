...
const TRANSACTIONS_PER_SUBBLOCK: u64 = 20;
...
impl<T: Transport + Clone, P: Provider<T, AnyNetwork> + Clone> HostExecutor<T, P> {
        ...
        V: Variant,
    {
        ...
        while current_block.body.len() as u64 > 0 {
            let cache_db = CacheDB::new(&rpc_db);
            let upper = std::cmp::min(current_block.body.len() as u64, TRANSACTIONS_PER_SUBBLOCK);
            let executor_output = V::execute_subblock(
                &executor_block_input,
                executor_difficulty,
                cache_db,
            ...
            tracing::info!("intermediate state root: {:?}", state_root);
